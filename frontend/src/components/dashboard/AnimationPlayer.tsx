...
export function AnimationPlayer({
  ...
  const overlay = (() => {
    if (isStreaming) return (
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-zinc-950 rounded-xl">
    ...
    if (isCompiling) return (
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-zinc-950 rounded-xl">
        <div className="w-9 h-9 rounded-full border-2 border-t-transparent animate-spin" style={{
          borderColor: '#7c3aed', borderTopColor: 'transparent',
    ...
    if (!Component) return (
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 text-center px-8 bg-zinc-950 rounded-xl">
        ...
        <div>
          ...
          <p className="text-zinc-500 text-xs leading-relaxed">Describe your animation in the chat</p>
          <p className="text-zinc-700 text-[11px] mt-1.5">e.g. &ldquo;A title card that says Hello World with a fade-in&rdquo;</p>
        </div>
      </div>
    );
